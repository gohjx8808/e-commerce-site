...
const PasswordRequirements = (props: PasswordRequirementsOwnProps) => {
  ...
  const toggleMet = useCallback(
    (targetKey: string, metRequirement: boolean) => {
      const targetEntryIndex = passwordRequirements.findIndex(
  ...
  const validatePassword = useCallback(() => {
    ...
    toggleMet(
      "passwordMatch",
      password !== undefined && password !== "" && password === rePassword
    );
    ...
    >
      <Grid container justifyContent="center" alignItems="center">
        ...
        {passwordRequirements.map((item) => (
          <ChecklistItem data={item} key={item.key} />
        ))}
      </Grid>
    </Grid>
  );
      ...
      <Grid container justifyContent="center">
        <Grid item xs={12} sm={9} md={7} lg={5}>
          <Grid container direction="row" alignItems="center">
            <Grid item>
              {meetsReq ? <DoneIcon htmlColor="#32cd32" /> : <CloseIcon />}
            </Grid>
            <Grid item>
